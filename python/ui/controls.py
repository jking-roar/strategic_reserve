...
class MenuView(tk.Frame):
    def __init__(self, parent: tk.Misc, on_new: Callable[[], None], on_quit: Callable[[], None]):
        ...
        tk.Label(self, text="Mode: Local two-player (PvP)").pack()
        unavailable = tk.Button(self, text="Computer opponent (unavailable)", state=tk.DISABLED)
        unavailable.pack(fill="x", pady=8)
...
class GameControls(tk.Frame):
    ...
    def render(self, state: GameState, message: str = "", animating: bool = False) -> None:
        ...
        can_roll = not animating and state.winner is None and dice is None
        can_pass = not animating and state.winner is None and dice is not None and not state.turn_context.legal_moves
